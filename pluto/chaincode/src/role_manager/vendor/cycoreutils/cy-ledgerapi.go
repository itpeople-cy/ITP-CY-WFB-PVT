...
var apiLogger = shim.NewLogger("ledger-api")
...
func UpdateObject(stub shim.ChaincodeStubInterface, objectType string, keys []string, objectData []byte, collectionName string) error {
	...
	apiLogger.Info("cycoreutils.UpdateObject() : Composite Key : ", compositeKey)
	...
	if err != nil {
		apiLogger.Errorf("cycoreutils.UpdateObject() : Error inserting Object into State Database %s", err)
		return err
	}
...
func DeleteObject(stub shim.ChaincodeStubInterface, objectType string, keys []string, collectionName string) error {
	...
	if err != nil {
		apiLogger.Errorf("cycoreutils.DeleteObject() : Error deleting Object into State Database %s", err)
		return err
	}
	apiLogger.Debug("cycoreutils.DeleteObject() : ", "Object : ", objectType, " Key : ", compositeKey)
...
func DeleteAllObjects(stub shim.ChaincodeStubInterface, objectType string) error {
	...
	err := stub.DelState(compositeKey)
	if err != nil {
		apiLogger.Errorf("cycoreutils.DeleteAllObjects() : Error deleting all Object into State Database %s", err)
		return err
	}
	apiLogger.Debug("DeleteAllObjects() : ", "Object : ", objectType, " Key : ", compositeKey)
...
func QueryObject(stub shim.ChaincodeStubInterface, objectType string, keys []string, collectionName string) ([]byte, error) {
	...
	apiLogger.Info("cycoreutils.QueryObject() : Compound Key : ", compoundKey)
...
func QueryObjectWithProcessingFunction(stub shim.ChaincodeStubInterface, objectType string, keys []string, fname func(shim.ChaincodeStubInterface, []byte, []string) error, collectionName string) ([]byte, error) {
	...
	compoundKey, _ := stub.CreateCompositeKey(objectType, keys)
	apiLogger.Info("cycoreutils.QueryObjectWithProcessingFunction: Compound Key : ", compoundKey)
	...
	apiLogger.Debug("fname() : Successful - Proceeding to fname")

	err = fname(stub, objBytes, keys)
	if err != nil {
		apiLogger.Error("cycoreutils.QueryObjectWithProcessingFunction() : Cannot execute  : ", fname)
...
func GetObjectHistory(stub shim.ChaincodeStubInterface, objectType string, keys []string) ([]byte, error) {
	...
	compositeKey, _ := stub.CreateCompositeKey(objectType, keys)
	apiLogger.Info("cycoreutils.GetObjectHistory() : Compound Key : ", compositeKey)
	...
	buffer.WriteString("]")
	apiLogger.Debugf("cycoreutils.GetObjectHistory(): Result:\n%s\n", buffer.String())
...
func GetKeyList(stub shim.ChaincodeStubInterface, args []string, collectionName string) (shim.StateQueryIteratorInterface, error) {
	...
	for i = 0; resultsIterator.HasNext(); i++ {
		...
		apiLogger.Debug("cycoreutils.GetKeyList() : my Value : ", myCompositeKey)
	}
...
func GetQueryResultForQueryString(stub shim.ChaincodeStubInterface, queryString string, collectionName string) ([]byte, error) {
	apiLogger.Infof("GetQueryResultForQueryString() : queryString:\n%s\n", queryString)
	...
	apiLogger.Debugf("cycoreutils.GetQueryResultForQueryString(): queryResult:\n%s\n", buffer.String())
...
func GetIteratorForQueryString(stub shim.ChaincodeStubInterface, queryString string, collectionName string) (shim.StateQueryIteratorInterface, error) {
	apiLogger.Infof("cycoreutils.GetIteratorForQueryString() : queryString:\n%s\n", queryString)
...
func GetList(stub shim.ChaincodeStubInterface, objectType string, keys []string, collectionName string) (shim.StateQueryIteratorInterface, error) {
	...
	apiLogger.Infof("cycoreutils.GetList(): Retrieving Objects into an array")
	if err != nil {
		return nil, err
	}
...
func VerifyAtLeastOneKeyIsPresent(args []string) error {
	...
	if nKeys < 1 {
		...
		apiLogger.Errorf("cycoreutils.VerifyAtLeastOneKeyIsPresent(): Error:%s", err)
