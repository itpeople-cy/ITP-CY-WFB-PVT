    ...
    getValue(name) {
        ...
        return new Promise(function (resolve, reject) {
            MongoClient.connect(self._url, {
                ...
                mongodb.collection(self._name).findOne({ "_id": name }).then(function (result) {
                    mongodb.close();
                    if (result) {
        ...
        return new Promise(function (resolve, reject) {
            MongoClient.connect(self._url, {
                ...
                mongodb.collection(self._name).findOne({ "_id": name }).then(function (result) {
                    ...
                    if (result) {
                        mongodb.collection(self._name).updateOne({ _id: name }, { $set: { member: value } }).then(function (result) {
                            mongodb.close(true);
                            resolve(true);
                        }).catch(function (error) {
                            mongodb.close(true);
                        ...
                        mongodb.collection(self._name).insertOne({ _id: name, member: value }).then(function (result) {
                            mongodb.close();
                            resolve(true);
                        }).catch(function (error) {
                            mongodb.close();
